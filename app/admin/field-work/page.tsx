...
export const metadata = { title: 'Field Work | Base Camp' }
...
export default async function FieldWorkAdminPage() {
  ...
  return (
    <div>
      ...
      {posts && posts.length > 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-line bg-ink-soft">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-[10px] font-700 uppercase tracking-widest text-khaki-deep">Slug</th>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-block rounded px-2 py-0.5 text-[10px] font-700 uppercase tracking-widest ${
                        post.published ? 'bg-olive/30 text-accent' : 'bg-ink-soft text-khaki-deep'
                      }`}
                    >
        ...
        <p className="text-sm text-khaki">No field notes yet. Create your first one.</p>
      )}
    </div>
  )
}
